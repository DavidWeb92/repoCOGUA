		...
		success: function(response){
			...
			for(let i = 0;i < response.length;i++){
				...
				if (response[i]["fields"]['imagen']){
					fila += '<td class="text-center fila-table"><a href="#" class="link" onclick="abrir_modal_detalles(\'/perfil_admin/detalles_hotel/'+response[i]['pk']+'/\');"><i class="fas fa-check" style="color: green;"></i></a></td>';
				}else{
					fila += '<td class="text-center fila-table"><a href="#" class="link" onclick="abrir_modal_detalles(\'/perfil_admin/detalles_hotel/'+response[i]['pk']+'/\');"><i class="fas fa-times" style="color: red;"></i></td>';
				}
				fila += '<td class="text-center fila-table"><button type="button" class="btn btn-danger btn-xs tableButton cambiar-color-button-eliminar" onclick="eliminarSweetAlertHotel(\''+response[i]['pk']+'\');"><i class="fas fa-trash"></i></button>';
...
function eliminarSweetAlertHotel(pk){
	Swal.fire({
	  title: 'Estas seguro?',
	  text: "Despues de eliminar el registro de la cabaña, no podras revertir los cambios!",
	  icon: 'warning',
