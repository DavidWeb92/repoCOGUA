...
function listarReservasPlatos(){
	$.ajax({
		url: "/perfil_admin/listar_reservas_platos/",
		type: "get",
		dataType: "json",
		success: function(response){
			if($.fn.DataTable.isDataTable('#tabla_reservas_platos')){
				$('#tabla_reservas_platos ').DataTable().destroy();
			}
			$('#tabla_reservas_platos tbody').html("");
			for(let i = 0;i < response.length;i++){
				...
				fila += '<td class="fila-table"><a href="#" class="link" onclick="abrir_modal_detalles(\'/perfil_admin/reserva_detalles_plato/'+response[i]['pk']+'/\');">' + (i+1) + '</a></td>';
				...
				fila += '<td class="fila-table"><a href="#" class="link" onclick="abrir_modal_detalles(\'/perfil_admin/reserva_detalles_plato/'+response[i]['pk']+'/\');">' + created +'</a></td>';
			...
			$('#tabla_reservas_platos').DataTable({
	...
	Swal.fire({
	  ...
	  if (result.isConfirmed) {
		...
		$.ajax({
			...
			url: '/perfil_admin/eliminar_reserva_plato/'+pk+'/',
			type: 'post',
			success: function(response){
				Swal.fire({
				  icon: 'success',
